...
public partial class resetTruckPassword : System.Web.UI.Page
{
    ...
    protected void resetPasswordButton_Click(object sender, EventArgs e)
    {
        ...
        SqlCommand cmd = new SqlCommand("select * from dbo.[truck] where ownerEmail=@userEmail and truckPIN=@userPIN", con);
        ...
        if (dt.Rows.Count > 0)
        {
            SqlCommand updatePass = new SqlCommand("update dbo.[truck] set ownerPassword=@userPass where ownerEmail=@userEmail", con);
            ...
            ClientScript.RegisterStartupScript(Page.GetType(), "validation", "<script language='javascript'> if(confirm('Your password has been changed!')){document.location = 'trucklogin.aspx'}</script>");
